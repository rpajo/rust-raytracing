...
fn main() {
    ...
    let vp_origin_left_bottom = camera_pos - (0.5 * vp_x_dir) - (0.5 * vp_y_dir) - Vec3::new(0.0, 0.0, focal_length);

    // export_ppm_image(255, 255);

    println!("P3");
    println!("{} {}", render_image_width, render_image_height);
    println!("255");
    for y in 0..render_image_height {
        ...
        for x in 0..render_image_width {
            ...
            let ray = Ray::new(camera_pos, vp_origin_left_bottom + (u * vp_x_dir) + (v * vp_y_dir) - camera_pos);

            let pixel_color = ray.ray_bg_color() * 255.0;
            println!("{} {} {}", pixel_color.x as i32, pixel_color.y as i32, pixel_color.z as i32);
        }
    }
}
